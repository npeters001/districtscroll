// This is a cleaned up version of public/script.js for the DistrictScroll project.
// It ensures that a visible placeholder card is present as soon as the page loads,
// defines a reusable constant for the SVG heart path, and uses that same path
// when rendering hearts for each brand card. The placeholder card is styled with
// a square aspect ratio so it is immediately visible in the grid layout.
...
(function () {
  ...
  // Define a single heart path constant.  Using a constant avoids duplicating
  // the long SVG path literal in multiple places and makes it easier to keep
  // the icon consistent. The minus signs in this path are contiguous to avoid
  // SVG parsing issues.
  const HEART_D =
    'M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 6.02 4.02 4 6.5 4c1.74 0 3.41 0.81 4.5 2.09C12.09 4.81 13.76 4 15.5 4 17.98 4 20 6.02 20 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z';
  ...
  // Insert placeholder card to ensure a card exists before data loads
  {
    ...
    if (container) {
      ...
      // Give the placeholder both the "card" class and an extra class for clarity.
      placeholder.className = 'card placeholder';
      // Give the placeholder a square aspect ratio so it has a visible height.
  ...
  function saveFilter() {
    localStorage.setItem(
      'ds_filter',
      JSON.stringify({ vibe: selectedVibe, cat: selectedCat }),
    );
  }
  ...
  function renderPills() {
    ...
    function pill(slug, label, type) {
      ...
      d.className =
        'pill' +
        ((type === 'vibe' && slug === selectedVibe) || (type === 'cat' && slug === selectedCat)
          ? ' active'
          : '');
      d.textContent = label;
      d.addEventListener('click', () => {
        if (type === 'vibe') selectedVibe = slug;
        else selectedCat = slug;
        saveFilter();
        updateURL();
        renderPills();
        renderCards();
